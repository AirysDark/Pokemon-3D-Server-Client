        ...
        public void Handle(Package p, Player Player = null)
        {
            ...
            if (this.MatchRequiredParam(p, Functions.CommandParamType.Any))
            {
                ...
                else if (Player == null)
                {
                    if (!Core.Player.HasPlayer(Group[0]))
                    {
                        Core.Logger.Log(Core.Setting.Token("SERVER_PLAYERNOTEXIST"), Pokemon_3D_Server_Core.Server_Client_Listener.Loggers.Logger.LogTypes.Info);
                    }
                    else
                    {
                        Player Players = Core.Player.GetPlayer(Group[0]);
                        ...
                        if (Players.IsOperator())
                        {
                            if (Players.GameJoltID == 116016 || Player.GameJoltID == 222452)
                            {
                                Core.Logger.Log($"You are not allowed to change or remove {PlayerName} operator status.", Pokemon_3D_Server_Core.Server_Client_Listener.Loggers.Logger.LogTypes.Info);
                            }
                            else
                            {
                                Players.RemoveOperator();

                                Core.Logger.Log($"You have successfully remove {PlayerName} as operator.", Pokemon_3D_Server_Core.Server_Client_Listener.Loggers.Logger.LogTypes.Info);
                            }
                        }
                        else
                        {
                            Core.Logger.Log(Core.Setting.Token("SERVER_NOTOPERATOR"), Pokemon_3D_Server_Core.Server_Client_Listener.Loggers.Logger.LogTypes.Info);
                        }
                    }
                }
            }
