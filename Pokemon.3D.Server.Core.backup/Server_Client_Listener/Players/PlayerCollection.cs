    ...
    public class PlayerCollection : List<Player>
    {
        ...
        public void Remove(int ID, string Reason)
        {
            ...
            if (Player.isGameJoltPlayer)
            {
                ...
                Core.Logger.Log(Core.Setting.Token("SERVER_GAMEJOLT", Player.Name, Player.GameJoltID.ToString(), "left the server with the following reason: " + Reason), Pokemon_3D_Server_Core.Server_Client_Listener.Loggers.Logger.LogTypes.Info);
            ...
            else
            {
                ...
                Core.Logger.Log(Core.Setting.Token("SERVER_NOGAMEJOLT", Player.Name, "left the server with the following reason: " + Reason), Pokemon_3D_Server_Core.Server_Client_Listener.Loggers.Logger.LogTypes.Info);
            }
            ...
            else
            {
                lock (Lock)
                {
                    try
                    {
                        ...
                        Writer.Flush();
                        Core.Logger.Log($"Sent: {p.ToString()}", Pokemon_3D_Server_Core.Server_Client_Listener.Loggers.Logger.LogTypes.Debug, p.Client);
                    }
                    catch (Exception) { }
                }
            }
