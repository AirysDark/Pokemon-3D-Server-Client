...
namespace Pokemon_3D_Server_Core.RCON_Client_Listener.Players
{
    ...
    public class PlayerCollection : List<Player>
    {
        ...
        public void Remove(int ID, string Reason)
        {
            ...
            Core.Logger.Log($"RCON Player (ID: {ID.ToString()}) have disconnected due to the following reason: {Reason}", Core.Logger.LogTypes.Info);
        ...
        public void Remove(TcpClient Client, string Reason)
        {
            ...
            Core.Logger.Log($"RCON Player (ID: {Player.ID.ToString()}) have disconnected due to the following reason: {Reason}", Core.Logger.LogTypes.Info);
            ...
            else
            {
                lock (Lock)
                {
                    try
                    {
                        ...
                        Writer.Flush();
                        Core.Logger.Log($"Sent: {p.ToString()}", Core.Logger.LogTypes.Debug, p.Client);
                    }
                    catch (Exception) { }
                }
            }
