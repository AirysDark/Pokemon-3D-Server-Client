...
namespace Pokemon_3D_Server_Core.Shared.jianmingyong
{
    ...
    public class Updater : IDisposable
    {
        ...
        public void Update()
        {
            ...
            {
                Core.Logger.Log("Unable to check for update.", Core.Logger.LogTypes.Info);
            }
        }
        ...
        private void ThreadUpdate()
        {
            try
            {
                Core.Logger.Log("Checking for update.", Core.Logger.LogTypes.Info);
                ...
                if (Core.Setting.ApplicationVersion != ExpectVersion)
                {
                    ...
                    Core.Logger.Log($"Update found: Expect Version: {ExpectVersion}, Current Version: {Core.Setting.ApplicationVersion}.", Core.Logger.LogTypes.Info);
                    Core.Logger.Log($"Downloading update.", Core.Logger.LogTypes.Info);
                    ...
                    Core.Logger.Log($"Download completed.", Core.Logger.LogTypes.Info);
                    Core.Logger.Log($"Checking Md5 checksum.", Core.Logger.LogTypes.Info);
                    ...
                    {
                        Core.Logger.Log($"MD5 does not match. Update failed.", Core.Logger.LogTypes.Info);
                    }
                }
                else
                {
                    Core.Logger.Log($"No update found. You are using the latest version.", Core.Logger.LogTypes.Info);
                }
            }
            catch (Exception)
            {
                Core.Logger.Log("Update failed. Please try again later.", Core.Logger.LogTypes.Info);
            }
            finally
            {
                Dispose();
