            ...
            do
            {
                try
                {
                    ...
                    if (LastWorldUpdate == null || LastWorldUpdate.AddHours(1) <= DateTime.Now)
                    {
                        ...
                        Core.Logger.Log(string.Format(@"Current Season: {0} | Current Weather: {1} | Current Time: {2}", GetSeasonName(Season), GetWeatherName(Weather), _CurrentTime.AddSeconds(TimeOffset).ToString()), Pokemon_3D_Server_Core.Server_Client_Listener.Loggers.Logger.LogTypes.Info);
                    }
