...
PokÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â©mon 3D Server Client Crash Log v {0}
--------------------------------------------------
...
[/CODE]",
                ...
                Core.Logger.Log(ex.Message + vbNewLine + "Error Log saved at: " + Core.Setting.ApplicationDirectory + "\\CrashLogs\\Crash_" + ErrorTime.Day.ToString() + "-" + ErrorTime.Month.ToString() + "-" + ErrorTime.Year.ToString() + "_" + ErrorTime.Hour.ToString() + "." + ErrorTime.Minute.ToString() + "." + ErrorTime.Second.ToString() + "." + RandomIndetifier + ".dat", Pokemon_3D_Server_Core.Server_Client_Listener.Loggers.Logger.LogTypes.Warning);
            }
            catch (Exception exc)
            {
                Core.Logger.Log(exc.Message, Pokemon_3D_Server_Core.Server_Client_Listener.Loggers.Logger.LogTypes.Warning);
            }
        }
