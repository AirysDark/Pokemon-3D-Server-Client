    ...
    public class Networking
    {
        ...
        private void ThreadPreHandlePackage(string p)
        {
            ...
            if (Package.IsValid)
            {
                ...
                Core.Logger.Log($"Receive: {Package.ToString()}", Pokemon_3D_Server_Core.Server_Client_Listener.Loggers.Logger.LogTypes.Debug, Client);
            }
        }
...
{
    Thread.Sleep((int)System.Math.Max(0, 1000 - sw.ElapsedMilliseconds));
}
sw.Restart();
            } while (IsActive);
        }
        ...
        private object ThreadSentToPlayer(object p)
        {
            try
            {
                ...
                Writer.Flush();
                Core.Logger.Log($"Sent: {((Package)p).ToString()}", Pokemon_3D_Server_Core.Server_Client_Listener.Loggers.Logger.LogTypes.Debug, Client);
            }
            catch (Exception) { }
